  ...
  return (
    <div className="cursor-pointer group card-lift" onClick={onOpen}>
      <div className="aspect-[3/4] bg-stone-100 rounded-2xl overflow-hidden mb-2.5 relative">
        ...
        <button onClick={add} className={`absolute bottom-2.5 right-2.5 w-9 h-9 rounded-full flex items-center justify-center transition-all duration-300 ${ok ? 'bg-green-700 text-white scale-110' : 'bg-white shadow-md text-stone-600 md:translate-y-2 md:opacity-0 md:group-hover:opacity-100 md:group-hover:translate-y-0'}`}>
          {ok ? I.check : I.plus}
        </button>
      </div>
      ...
      <div className="flex items-center justify-between mt-1.5">
        ...
        <button onClick={add} className={`text-[10px] font-bold px-3 py-1.5 rounded-lg transition-all duration-200 btn-press ${ok ? 'bg-green-700 text-white' : 'bg-green-800 text-white hover:bg-green-700'}`}>{ok ? '✓ Added' : 'Add'}</button>
      </div>
  ...
  return (
    <div className="min-h-screen bg-white pb-16 md:pb-0" style={{ fontFamily: 'var(--font-body)' }}>
      ...
      <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-stone-100">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-14 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-6">
            {[['Home','home','/'],['Shop','shop','/shop'],['Track','track','/track']].map(([l,p,h]) => <button key={p} onClick={() => go(p,h)} className={`text-xs font-semibold transition ${page === p ? 'text-green-800' : 'text-stone-400 hover:text-stone-600'}`}>{l}</button>)}
          </div>
                ...
                return (
                  <div key={b.id} className="bg-gray-50 rounded-xl overflow-hidden">
                    <div className="aspect-[4/3] bg-gray-100 relative">
                      ...
                      <span className="absolute top-2 left-2 px-2 py-0.5 bg-green-800 text-white text-[8px] font-bold rounded-md tracking-wide">BUNDLE</span>
                    </div>
                    <div className="p-2.5">
      ...
      {page === 'shop' && <div className="max-w-7xl mx-auto px-4 sm:px-6 py-5">
        ...
        <div className="flex gap-2 overflow-x-auto scrollbar-hide mb-5">{cats.map(c => <button key={c} onClick={() => setCat(c)} className={`h-8 px-4 rounded-lg text-[11px] font-semibold whitespace-nowrap shrink-0 transition ${cat === c ? 'bg-green-800 text-white' : 'bg-gray-50 text-gray-500 hover:bg-gray-100'}`}>{c === 'all' ? 'All' : c}</button>)}</div>
        ...
        <div className="grid md:grid-cols-2 gap-5 md:gap-10">
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="flex gap-2 items-center mb-3">
              ...
              <button onClick={() => addToCart(sel)} className="flex-1 h-11 bg-green-800 text-white rounded-lg text-sm font-semibold hover:bg-green-700 transition">Add to Cart</button>
          ...
          <div className="space-y-2.5 mb-5">{cart.map(c => <div key={c.id} className="flex gap-3 items-center p-3 rounded-xl bg-stone-50 border border-stone-100 relative group">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-1">
                ...
                {c.isWholesale && <span className="text-[8px] font-bold bg-green-50 text-green-700 px-1.5 py-0.5 rounded-full">WHOLESALE</span>}
              </div>
            </div>
            <div className="flex items-center gap-1"><button onClick={() => updQty(c.id,-1)} className="w-7 h-7 rounded-lg bg-white flex items-center justify-center text-stone-600 border border-stone-200 hover:border-green-600 transition">{I.minus}</button><span className="w-6 text-center text-[12px] font-bold text-stone-900">{c.qty}</span><button onClick={() => updQty(c.id,1)} className="w-7 h-7 rounded-lg bg-white flex items-center justify-center text-stone-600 border border-stone-200 hover:border-green-600 transition">{I.plus}</button></div>
          </div>)}</div>
          <div className="bg-green-900 text-white rounded-xl p-4 mb-4 flex justify-between items-center"><span className="text-sm text-green-300">Total</span><span className="text-lg font-bold">{money(ct)}</span></div>
          <button onClick={() => { setCheckoutStep(1); setPage('checkout'); window.location.hash = '/checkout' }} className="w-full h-12 bg-green-800 text-white rounded-full text-sm font-bold hover:bg-green-700 transition btn-press flex items-center justify-center gap-2">Proceed to Checkout {I.arrow}</button>
      ...
      {page === 'checkout' && <div className="max-w-md mx-auto px-4 sm:px-6 py-6">
          <button onClick={() => go('cart','/cart')} className="text-[11px] text-gray-400 hover:text-green-800 mb-5 inline-flex items-center gap-1">&larr; Back to cart</button>
          ...
          <button onClick={placeOrder} disabled={!custName.trim() || !custPhone.trim() || (fulfillment === 'delivery' && !custAddress.trim()) || submitting} className="w-full h-12 bg-green-800 text-white rounded-full text-sm font-bold hover:bg-green-700 transition btn-press disabled:opacity-30 flex items-center justify-center gap-2">
        ...
        <div className="max-w-sm mx-auto px-5 py-12">
          ...
          {!PAYMENTS_ENABLED ? (
            <>
              <div className="flex items-center gap-3 mb-6">
                <div className="w-9 h-9 rounded-full bg-green-800 flex items-center justify-center flex-shrink-0">
            ...
            <>
              ...
              <button onClick={resendPrompt} disabled={retrying || retryWait > 0} className="w-full h-11 bg-green-800 text-white rounded-lg text-sm font-medium disabled:opacity-40 mb-3">
                {retrying ? 'Sending…' : retryWait > 0 ? `Send again in ${retryWait}s` : 'Send request again'}
              </button>
          ...
          <div className="border-t border-gray-100 pt-5 space-y-2.5">
            ...
            <div className="flex justify-between text-sm">
              ...
              <a href="#/track" onClick={() => go('track','/track')} className="font-medium text-green-800">Track order</a>
            </div>
            <div className="flex justify-between text-sm">
      ...
      {/* ═══ FOOTER ═══ */}
      <footer className="bg-green-900 mt-12 mb-16 md:mb-0">
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="py-8 grid grid-cols-2 sm:grid-cols-4 gap-6">
            ...
            <div>
              ...
              <div className="space-y-2">
                ...
                <a href={`https://wa.me/${WA}`} target="_blank" className="block text-[12px] text-green-400 hover:text-green-300 transition">WhatsApp Us</a>
              </div>
            </div>
            <div>
          ...
          <div className="border-t border-green-800/50 py-4">
            <p className="text-[10px] text-green-700">&copy; {new Date().getFullYear()} BEDTIME BEDDINGS HOME. All rights reserved.</p>
          </div>
        </div>
      </footer>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-stone-100 z-50 px-2 pb-[env(safe-area-inset-bottom)]">
        <div className="flex justify-around h-[58px] items-center">
          {[
            ...
            <button key={p} onClick={() => go(p,h)} className={`flex flex-col items-center gap-1 relative transition-colors ${page===p?'text-green-800':'text-stone-400'}`}>
              <span className="relative">{ic}{p==='cart' && cc > 0 && <span className="absolute -top-1.5 -right-2 min-w-[16px] h-4 bg-green-800 text-white text-[8px] font-bold rounded-full flex items-center justify-center px-1">{cc}</span>}</span>
              <span className={`text-[9px] ${page===p?'font-bold':'font-medium'}`}>{l}</span>
              {page===p && <span className="absolute -top-[1px] left-1/2 -translate-x-1/2 w-5 h-[2px] bg-green-800 rounded-full" />}
            </button>
          ))}
        </div>
      </div>
